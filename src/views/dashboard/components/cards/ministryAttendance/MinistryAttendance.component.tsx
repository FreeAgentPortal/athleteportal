...
const MinistryAttendance = () => {
  ...
  const { data: selectedProfile } = useApiHook({
    url: `/ministry/${loggedInData?.ministry?._id}`,
    key: 'selectedProfile',
    enabled: !!loggedInData?.ministry?._id,
    method: 'GET',
  }) as any;
  ...
  const allLocationTypes: string[] = Array.from(new Set(attendanceData?.payload?.flatMap((entry: any) => Object.keys(entry.checkIns || {}))));

  return (
    <div className={styles.container}>
      <div className={styles.topContainer}>
        <p>
          <strong>{selectedProfile?.ministry?.name}&apos;s</strong> attendance records between {new Date(endDate).toLocaleDateString()} - {new Date(startDate).toLocaleDateString()}
        </p>
      ...
      <ResponsiveContainer width="100%" height={400}>
        <LineChart data={transformedData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          {allLocationTypes.map((location, index) => (
            <Line key={index} type="monotone" dataKey={location} stroke={`#${Math.floor(Math.random() * 16777215).toString(16)}`} strokeWidth={2} dot={{ r: 3 }} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    </div>
